...
# 命令列表（你的命令）
COMMANDS=(
    "cd ~/Fast-Drone-250 && source devel/setup.bash && roslaunch realsense2_camera rs_camera.launch"
    "cd ~/Fast-Drone-250 && source devel/setup.bash && roslaunch vins fast_drone_250.launch"
    "cd ~/vins_ws && source devel/setup.bash && roslaunch vins_to_mavros vins_to_mavros.launch"
)
