#!/bin/bash

# 存储进程组ID
declare -A PGROUPS

# 启动命令并检查
start_command_and_check() {
    local cmd="$1"

    # 不写日志，直接在终端打印
    setsid bash -c "$cmd" &
    local sid=$!

    sleep 3

    if kill -0 $sid >/dev/null 2>&1; then
        echo "Command started successfully: $cmd"
        PGROUPS["$cmd"]=$sid
    else
        echo "Failed to start command: $cmd"
        exit 1
    fi
}

# 清理函数
cleanup() {
    echo "Stopping all commands..."
    for cmd in "${!PGROUPS[@]}"; do
        local sid=${PGROUPS["$cmd"]}
        echo "Stopping $cmd (PID $sid)..."
        kill -- -$sid 2>/dev/null
    done
    wait
    echo "All commands stopped"
    exit
}

# 注册退出处理
trap cleanup EXIT INT TERM

# 命令列表（你的命令）
COMMANDS=(
    "cd ~/lio_ws && source devel/setup.bash && roslaunch livox_ros_driver2 msg_MID360.launch "
    "cd ~/lio_ws && source devel/setup.bash && roslaunch fast_lio mapping_mid360.launch"
    "cd ~/odom_to_mavros && source devel/setup.bash && roslaunch odom_to_mavros odom_to_mavros.launch"
)

# 启动所有命令
for CMD in "${COMMANDS[@]}"; do
    start_command_and_check "$CMD"
done

# 等待所有命令结束
while true; do
    sleep 60
done
